...
dbconn = MongodbConnection(username="root", password="rootAtlas")
list_cursor = dbconn.getData(
    dbName="Algerian_Forest-Fires", collectionName="Forest-Fires"
)
    ...
    try:
        ...
        if output > 12:
            return render_template(
                ...
                reg_prediction="Fire Weather Index is {:.4f} ---- Forest is in Danger !".format(
                    output
                ),
            )
        else:
            return render_template(
                "single_prediction.html",
                reg_prediction="Fire Weather Index is {:.4f} ---- Forest is Safe !".format(
                    output
                ),
            )
        ...
        return render_template(
            "single_prediction.html", reg_prediction="Check the Input again!!!"
        )
...
def single_class_predict():
    try:
        ...
        log.error("Input error, check input", e)
        return render_template(
            "single_prediction.html", class_prediction="Check the Input again!!!"
        )
...
def batch_class_predict():
    try:
        ...
        input_value = [int(x) for x in request.form.values()][0]
        # Retrieves the dataframe with complete prediction details
        frame = RandomBatchModeling.outframe(
            list_dict, input_value, scaler, class_model
        )
